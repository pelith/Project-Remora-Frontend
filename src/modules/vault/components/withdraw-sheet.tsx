...
import { useVaultWithdraw } from '@/modules/contracts/hooks/use-vault-withdraw';
...
export const WithdrawSheet = ({
	...
	const { withdraw: withdrawToken0, isPending: isWithdrawingToken0 } =
		useVaultWithdraw(vaultAddress);
	const { withdraw: withdrawToken1, isPending: isWithdrawingToken1 } =
		useVaultWithdraw(vaultAddress);

	// Get vault's token balances (available for withdrawal)
	const token0Info = useTokenInfoAndBalance(vaultAddress, token0Address);
	const token1Info = useTokenInfoAndBalance(vaultAddress, token1Address);
	...
	const handleWithdrawToken0 = () => {
		...
		if (amountBign.gt(token0Info?.balance ?? 0)) {
			setErrors((prev) => ({
				...prev,
				token0: 'Exceeds available balance',
			}));
			return;
		}
		...
		withdrawToken0({
			...
			amountRaw: parseUnits(amountBign.toString(), token0Info?.decimals ?? 18),
			to: address ?? '',
		});
	};

	const handleWithdrawToken1 = () => {
		// Validate amount
		...
		if (amountBign.gt(token1Info?.balance ?? 0)) {
			setErrors((prev) => ({
				...prev,
				token1: 'Exceeds available balance',
			}));
			return;
		}
		...
		withdrawToken1({
			...
			amountRaw: parseUnits(amountBign.toString(), token1Info?.decimals ?? 18),
			to: address ?? '',
		});
	};
		...
		>
			...
			<SheetContent>
				<SheetHeader>
					...
					<SheetDescription>
						Withdraw funds from your vault to your wallet.
					</SheetDescription>
				</SheetHeader>
				...
				<div className='py-6 space-y-6'>
					<div className='space-y-2'>
						...
						<Button
							...
							disabled={!isConnected || isWithdrawingToken0}
						>
							Confirm Withdraw {token0Info?.symbol}
						</Button>
					</div>
					...
					<div className='space-y-2'>
						...
						<Button
							className='w-full'
							onClick={handleWithdrawToken1}
							disabled={!isConnected || isWithdrawingToken1}
						>
							Confirm Withdraw {token1Info?.symbol}
						</Button>
					</div>
				</div>
			</SheetContent>
